...
- `backdrop-blur-sm` on the Skills `SKILL_GROUPS` cards. Same class of bug as `.project-card`: 6 `motion.article` blur surfaces with no breakpoint gate, all compositing simultaneously on every scroll frame. Removed; compensated by bumping each group's `accent` gradient-stop opacities ~5-10% so the card still reads as a distinct tinted surface without blur. **`backdrop-filter` is now only acceptable on small/short-lived elements gated off desktop** (e.g. mobile sticky headers, which drop it at `lg:`) **or tiny fixed elements** (e.g. `LanguageToggle`) — never on a grid of 4+ simultaneously-visible cards.
